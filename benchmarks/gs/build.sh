...
X86GCCFLAGS="-fopenmp-simd -fargument-noalias -funroll-loops"
X86ICCFLAGS="-qopenmp-simd -fno-alias -unroll"
...
if [[ $ARCH == 0 ]]; then
    ...
    if [[ $ALL == 1 ]]; then
        ...
        # GCC --> CC=gcc/gfortran
        $CC $X86GCCFLAGS -march=skylake-avx512 -Ofast -c $KERNEL.f90
        $CC $X86GCCFLAGS -march=skylake-avx512 -Ofast -S $KERNEL.f90 -o $KERNEL.s.csx.gcc.s
        $CC $KERNEL.o timing.o -lm -o a.$KERNEL.csx.gcc
    fi
elif [[ $ARCH == 1 ]]; then
    # Compile for Zen 
    $GNUC $X86GCCFLAGS -march=znver1 -mavx2 -mfma -Ofast -c $KERNEL.f90
    $GNUC $X86GCCFLAGS -march=znver1 -mavx2 -mfma -Ofast -S $KERNEL.f90 -o $KERNEL.s.zen.gcc.s
    $GNUC $KERNEL.o timing.o -lm -o a.$KERNEL.zen.gcc
elif [[ $ARCH == 2 ]]; then
    # Compile for ThunderX2
    if [[ $ALL == 1 ]]; then
        # Compile all
        $ARMC $ARMFLAGS -Ofast -c $KERNEL.f90
        $ARMC $ARMFLAGS -Ofast -S $KERNEL.f90 -o $KERNEL.s.tx2.clang.s
        $ARMC $KERNEL.o timing.o -lm -o a.$KERNEL.tx2.clang
        $GNUC $ARMFLAGS -Ofast -c $KERNEL.f90
        $GNUC $ARMFLAGS -Ofast -S $KERNEL.f90 -o $KERNEL.s.tx2.gcc.s
        $GNUC $KERNEL.o timing.o -lm -o a.$KERNEL.tx2.gcc
    elif [[ $CC == arm* ]]; then
        # ARM HPC Compiler --> CC=armclang/armflang
        $CC $ARMFLAGS -Ofast -c $KERNEL.f90
        $CC $ARMFLAGS -Ofast -S $KERNEL.f90 -o $KERNEL.s.tx2.clang.s
        $CC $KERNEL.o timing.o -lm -o a.$KERNEL.tx2.clang
    else
        # GCC --> CC=gcc/gfortran
        $CC $ARMFLAGS -Ofast -c $KERNEL.f90
        $CC $ARMFLAGS -Ofast -S $KERNEL.f90 -o $KERNEL.s.tx2.gcc.s
