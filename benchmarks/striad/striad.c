...
void striad(long size) {
  ...
  printf("%12.1f | %11.8f | %9.3f | %7.2f | %7.1f | %7.1f | %6d | %4ld \n", (double)size*sizeof(double)/1000.0, runtime, 2.0*(double)size*(double)repeat*1e-6/runtime, runtime*(double)freq/((double)(size*repeat)/8.0),24.0*(double)repeat*(double)size/runtime/1024.0/1024.0, (double)repeat*(double)size/runtime/1000.0/1000.0, repeat, size);
...
{ 
  ...
  printf("SCHOENAUER TRIAD a[i] = b[i] + c[i] * d[i], 24 byte/it, 2 Flop/it\n");
