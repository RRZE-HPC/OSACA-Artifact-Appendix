...
if [[ $ARCH == 0 ]]; then
    ...
    if [[ $ALL == 1 ]]; then
        ...
        $GNUC $X86GCCFLAGS -march=skylake-avx512 -O3 -c $KERNEL.c
        $GNUC $X86GCCFLAGS -march=skylake-avx512 -O3 -S $KERNEL.c -o $KERNEL.s.csx.gcc.O3.s
        $GNUC $KERNEL.o timing.o -lm -o a.$KERNEL.csx.gcc.O3
    elif [[ $CC == i* ]]; then
        # ICC --> CC=icc/ifort
        ...
        $CC $X86GCCFLAGS -march=skylake-avx512 -Ofast -c $KERNEL.c
        $CC $X86GCCFLAGS -march=skylake-avx512 -Ofast -S $KERNEL.c -o $KERNEL.s.csx.gcc.O3.s
        $CC $KERNEL.o timing.o -lm -o a.$KERNEL.csx.gcc.O3
    fi
elif [[ $ARCH == 1 ]]; then
    # Compile for Zen 
    ...
    $CC $X86GCCFLAGS -march=znver1 -mavx2 -mfma -Ofast -c $KERNEL.c
    $CC $X86GCCFLAGS -march=znver1 -mavx2 -mfma -Ofast -S $KERNEL.c -o $KERNEL.s.zen.gcc.O3.s
    $CC $KERNEL.o timing.o -lm -o a.$KERNEL.zen.gcc.O3
elif [[ $ARCH == 2 ]]; then
    ...
    if [[ $ALL == 1 ]]; then
        ...
        $GNUC $ARMFLAGS -Ofast -c $KERNEL.c
        $GNUC $ARMFLAGS -Ofast -S $KERNEL.c -o $KERNEL.s.tx2.gcc.O3.s
        $GNUC $KERNEL.o timing.o -lm -o a.$KERNEL.tx2.gcc.O3
    elif [[ $CC == arm* ]]; then
        ...
        $CC $ARMFLAGS -Ofast -c $KERNEL.c
        $CC $ARMFLAGS -Ofast -S $KERNEL.c -o $KERNEL.s.tx2.gcc.O3.s
        $CC $KERNEL.o timing.o -lm -o a.$KERNEL.tx2.gcc.O3
    fi
fi
