...
void scale(long size, double scale) {
  ...
  printf("%12.1f | %11.8f | %9.3f | %7.2f | %7.1f | %7.1f | %6d | %4ld \n", (double)size*sizeof(double)/1024.0, runtime, 1.0*(double)size*(double)repeat*1e-6/runtime, runtime*(double)freq/((double)(size*repeat)/8.0),8.0*(double)repeat*(double)size/runtime/1024.0/1024.0, (double)repeat*(double)size/runtime/1000.0/1000.0, repeat, size);
...
{ 
  ...
  printf("VECTOR UPDATE a[i] = s * a[i], 8 byte/it, 1 Flop/it\n");
